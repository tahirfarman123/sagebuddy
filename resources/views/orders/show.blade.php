        ...
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-lg-4 col-md-3 col-sm-12">
                    ...
                    <p class="m-0 text-mute breadc-text"> Order
                            ...
                            <table class="table table-borderless">
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td style="float: right;">{{date('F d, Y',
                                            strtotime($order['created_at_date']))}}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
        ...
        <div class="col-lg-8 items_card">
            <div class="card">
                <div class="card-body">
                    ...
                    @if($order['line_items'] && is_array($order['line_items']) &&
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <small class="text-muted">Variant: {{$item['variant_title'] ??
                                        ''}}</small> <br>
                                </td>
                                ...
                                <td>
                                    @php $sub_price = number_format((double) $item['price'] * (double)
                                    $item['quantity'], 2); @endphp
                                ...
                                <td style="display: none;" class="fulfill-th">
                                    ...
                                    $item['fulfillment_service'] === 'app-fulifllment-service')
                                    <a href="#" class="btn btn-primary fulfill_this_item"
                                        data-line_item_id="{{$item['id']}}" data-qty="{{$item['quantity']}}">Fulfill</a>
                                    @else
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-truncate text-right"><span style="float:right">-
                                        {{$order_currency.' '.number_format($discount, 2)}}</span></td>
                            </tr>
                            @endforeach
                            ...
                            <tr>
                                ...
                                <td class="text-truncate text-right"><span style="float:right">-
                                        {{$order_currency.' '.number_format($order->total_discounts,
                                        2)}}</span>
                                </td>
                            </tr>
                            @endif
                            ...
                            <tr>
                                ...
                                <td class="text-truncate text-left">{{strlen($ship['title']) < 20 ? $ship['title']
                            ...
                            <tr>
                                ...
                                <td class="text-truncate text-left">{{$tax['title'].' (
                                    '.((float)$tax['rate'] *
                                    100).'% )'}}</td>
                                <td class="text-truncate text-right"><span style="float:right">{{$order_currency.'
                                        '.number_format($tax['price'],
                                        2)}}</span></td>
                            </tr>
                            @endforeach
                            @if(!empty($order['tax_lines']) && count($order['tax_lines']) > 1)
                            <tr>
                                <td colspan="2" class="text-truncate">Total Taxes
                                    @if($order['taxes_included']
                                    == true) (Inclusive) @endif</td>
                                <td class="text-truncate text-left"></td>
                                <td class="text-truncate text-right"><span style="float:right">{{$order_currency.'
                                        '.number_format($order['total_tax'], 2)}}</span></td>
                            </tr>
                            @endif
                            @endif
                            <tr>
                                ...
                                <td class="text-truncate text-right text-bold"><span
                                        style="float:right">{{$order_currency.' '}}{{number_format(
                                        $order['total_price'], 2)}}</span></td>
                            </tr>
                        </tbody>
                    </table>
                    @endif
...
<script>
    $(document).ready(function () {
            ...
            $('.fulfill_submit').click(function (e) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{route('shopify.order.single.fulfill')}}",
                    data: data,
                    async: false,
