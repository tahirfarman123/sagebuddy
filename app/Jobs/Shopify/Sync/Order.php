...
class Order implements ShouldQueue
{
    ...
    public function handleWithRESTAPI()
    {
        try {
            ...
            do {
                $orders_payload = [];
                // $endpoint = getShopifyURLForStore('orders.json?limit=250&fulfillment_status=unshipped&since_id=' . $since_id, $this->store);
                $endpoint = getShopifyURLForStore('orders.json?limit=250&since_id=' . $since_id, $this->store);
    ...
    private function insertOrders($ordersTableString)
    {
        try {
            ...
            DB::insert($query);
            toastr()->success('' . $this->store->name . 'Order sync successful!');
            return true;
        } catch (\Exception $e) {
